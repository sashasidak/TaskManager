...
let testCaseAreaLocator = "#test_case_col";
...
function renderTestCaseCreateForm(parent_test_suite_id) {
    $(testCaseAreaLocator).load(`/tc/${parent_test_suite_id}/create`, function() {
        $('textarea').autoResize();
    });
}
...
function getTestCaseDataFromForm() {
    ...
    testCase.need_on_smoke = $("#tcc_need_on_smoke_select").val();
...
function createTestCase() {
    ...
    $.ajax({
        ...
        data: {
            ...
            'need_on_smoke': newTestCase.need_on_smoke,
            'data': JSON.stringify(newTestCase.data)
        },

        success: function (data) {  // response is case html and json
            $("#test_case_viewer").remove();
            $(`[data-suite_id='${newTestCase.suite_id}'] > .tree_suite_test_cases`).append(data.html);
...
function updateTestCase() {
    ...
    $.ajax({
        ...
        data: {
            ...
            'need_on_smoke': updatingTestCase.need_on_smoke,
